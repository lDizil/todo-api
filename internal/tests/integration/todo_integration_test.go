...
func TestCreateTodo_Integration(t *testing.T) {
	...
	qeury := "SELECT id, task_name, description, completed, created_at FROM todos WHERE id=$1"
	err = db.QueryRow(qeury, response.ID).Scan(
		&todoInDB.ID,
		&todoInDB.TaskName,
...
func TestGetAllTask_Integration(t *testing.T) {
	...
	assert.NotEmpty(t, response[0].TaskName)
	assert.NoError(t, err)
